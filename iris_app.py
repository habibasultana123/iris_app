...
def prediction(model, sepal_length, sepal_width, petal_length, petal_width):
  species = model.predict([[sepal_length_cm, sepal_width_cm, petal_length_cm, petal_width_cm]])
  species = species[0]
  if species == 0:
    return "Iris-setosa"
  elif species == 1:
...
s_len = st.sidebar.slider("Sepal Length", float(iris_df["sepal_length_cm"].min()), float(iris_df["sepal_length_cm"].max()))
# The 'float()' function converts the 'numpy.float' values to Python float values.
s_wid = st.sidebar.slider("Sepal Width", float(iris_df["sepal_width_cm"].min()), float(iris_df["sepal_width_cm"].max()))
p_len = st.sidebar.slider("Petal Length", float(iris_df["petal_length_cm"].min()), float(iris_df["petal_length_cm"].max()))
p_wid = st.sidebar.slider("Petal Width", float(iris_df["petal_width_cm"].min()), float(iris_df["petal_width_cm"].max()))
